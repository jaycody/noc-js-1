var pageTitle = "noc_p5_gungfu-01_vectors-02_bounce_vectors";
...
function draw(){
  //DRAW MERIDIANS if more than 2 walkers exist (draw underneath the points)  
  if (walkers.length >= 4) {
    //println("walkers.length = " + walkers.length);
    meridian.drawLine();
  }
...
function Meridian(){
  this.meridianScale = random(.001, .1);

  this.drawLine = function () {
    //set stroke info based on the most recently created perlin point
    //strokeFromWalker = walkers.length-1;
    
    //use stroke info from the top ellipse from the first perlin point
    strokeFromWalker = circlesPerClick-1;

    strokeWeight(walkers[strokeFromWalker].circleSizeVector.x*this.meridianScale);
    stroke(walkers[strokeFromWalker].colorVector.x, walkers[strokeFromWalker].colorVector.y, walkers[strokeFromWalker].colorVector.z,255);
    fill(walkers[strokeFromWalker].colorVector.x, walkers[strokeFromWalker].colorVector.y, walkers[strokeFromWalker].colorVector.z,walkers[strokeFromWalker].fillAlpha);
      
    

    beginShape();
    //println("walkers.length = " + walkers.length);
    //println("meridian strokeWeight = " + walkers[strokeFromWalker].circleSizeVector.x*this.meridianScale);
    //println("meridianScale = " + this.meridianScale);
    for (var i = circlesPerClick-1; i < walkers.length; i = i + circlesPerClick){
      //print(i);
      vertex(walkers[i].location.x, walkers[i].location.y);
      //vertex(walkers[i].location.x, walkers[i].location.y);
      //line(x1, y1, x2, y2);
    }  
    //noFill();
...
function Walker(x, y, isEllipse) {
  ...
  this.noiseToOffsetLocation = createVector(random(255), random(255));
  this.incrementNoiseLocation= createVector(random(.001, .01),random(.001, .01));//0.004;
  ...
  this.circleSizeScalar         = random(-.5,.5);
  ...
  this.colorize = function(){
    ...
    this.strokeAlpha   = map(noise(this.noiseToOffsetStrokeAlpha),0,1,0,100);//50
    ...
    this.fillAlpha     = map(noise(this.noiseToOffsetFillAlpha),0,1,0,50);//8
  ...
  this.walk = function(){
    ...
    //check boundary and reverse if needed
    ...
    /*
    for (var i = 0; i < walkers.length; i++){
      this.velocity.x += map(noise(this.noiseToOffsetLocation.x),0,1,-.1,.1);
      this.velocity.y += map(noise(this.noiseToOffsetLocation.y),0,1,-.1,.1);
    }
    //then increment the noise vector by using vector math's ADD function
    this.noiseToOffsetLocation.add(this.incrementNoiseLocation.x,this.incrementNoiseLocation.y);
    */
