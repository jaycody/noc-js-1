function Vehicle() {
  ...
  this.perlinForce  = createVector(random(10), random(10));
  ...
  this.perlinForceRange     = createVector(4, 4);  //(3,3)
  //this.perlinForceRange     = createVector(.3,.3);
  
  //this.perlinForceRange     = createVector(2, 2);  //(3,3)
  
  //this.perlinForceIncrement = createVector(random(.3),random(.3)); //(.03, .03)
  this.perlinForceIncrement = createVector(.009,.009);
  ...
  this.perlinSteer  = function(){
    ...
    //var f = createVector(map(noise(this.perlinForce.x),0,1, -(this.perlinForceRange.x*.8), this.perlinForceRange.x), map(noise(this.perlinForce.y),0,1, -this.perlinForceRange.y, this.perlinForceRange.y));
    
    //var f = createVector(map(noise(this.perlinForce.x),0,1, -7.5, 10), map(noise(this.perlinForce.y),0,1, -7.5, 10));
  ...
  this.displayHistory = function(){
    ...
    //fill(0,255,0,20);
    fill(this.fillVector.x,this.fillVector.y,this.fillVector.z,20);
    endShape(CLOSE);
  };
  ////END HISTORY 
  /////////////////////////////////////////////////////
